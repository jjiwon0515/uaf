...
-----------------------------------------------------------------------------3일차-------------------------------------------------------------------
해당 환경에서 파일이 만들어지지 않는다. 그래서 파이썬 pwn 모듈로 파일을 생성해서 문제를 해결하려고 했는데, 방법을 찾기 어려웠다. 그래서 <리눅스 임시 파일 생성>에 대해서 찾아보니 mktemp라고 /tmp 폴더 안에서 파일을 만드는 명령어를 찾을 수 있었다. 그래서 그냥 직접 tmp 폴더안에서 파일을 만들어봤다. 성공적으로 만들 수 있었다. c언어 구조체에서 new를 통한 동적할당을 할 수 있고 delete가 free와 같은 역할을 한다는 것을 찾을 수 있었다. 코드 내용과 그에 대한 해석을 다시 해봐야겠다.

코드에 대한 해석을 전부 끝냈다. 다시 디버깅해서 /tmp 폴더안에 0x000000000040118b를 넣고 동적할당을 할때 몇 byte를 가져가서 할당하는지 알아봐야 문제를 해결할 수 있을 것 같다
조금만 더 힘내면 문제를 풀 수 있을 거 같다!!
